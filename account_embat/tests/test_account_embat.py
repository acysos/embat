...
class TestAccountEmbat(TransactionCase):
    def setUp(self):
        ...
        self.test_account = self.env['account.account'].search([('code', '=', '999999'), ('company_ids', 'in', self.company.id)], limit=1)
        ...
        self.test_partner.property_account_receivable_id = receivable_account.id

        # Ensure an income account exists
        income_account = self.env['account.account'].search([('account_type', '=', 'income'), ('company_ids', 'in', self.company.id)], limit=1)
        ...
        sales_journal = self.env['account.journal'].search([('type', '=', 'sale'), ('company_id', '=', self.company.id)], limit=1)
