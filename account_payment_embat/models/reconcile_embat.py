...
class EmbatAccount(models.Model):
    ...
    default_payment_account = fields.Many2one(
        comodel_name="account.account", string="Default Payment Account",
        required=True)
    ...
    def reconcile_payments_embat(self):
        ...
        for company in companies:
            ...
            if not embat_data_ids:
                _logger.info(
                    "No Embat data configured for the company %s.",
                    company.name)
                continue
            for embat_data in embat_data_ids:
                ...
                response, content = embat_data._embat_request(
                    endpoint, embat_data, request_type=request_type,
                    params=params)
                if not content or "data" not in content:
                    message = _(
                        "Could not retrieve the payments from Embat: %s") % (
                            response)
                    embat_data._create_log(
                        "ERROR", "PAYMENTS_ERROR", message, embat_data)
                    continue
    ...
    def get_payment_operation(self, payment, company):
        ...
        for operation in payment["operations"]:
            move = move_env.search(
                [("id", "=", int(operation["customId"].split("-")[1]))])
            ...
            if move.payment_state == "in_payment":
                ...
                if move_payment:
                    ...
                    if (move.embat_transaction_id and
                            payment["transactionId"] not in
                            move.embat_transaction_id.split(",")):
                    ...
                    move_payment = self.env["account.payment.register"].with_context(
                        ...
                        active_ids=move.ids).create(payment_vals)._create_payments()
                    move_payment.embat_id = payment["transactionId"]
            ...
            move_vals = {
                ...
                "line_ids": [
                    (0, 0, {
                        ...
                        "credit": payment["paymentAmount"]*-1 if payment["paymentAmount"] < 0 else 0,
                        "debit": payment["paymentAmount"] if payment["paymentAmount"] > 0 else 0,
                    ...
                    (0, 0, {
                        ...
                        "credit": payment["paymentAmount"] if payment["paymentAmount"] > 0 else 0,
                        "debit": payment["paymentAmount"]*-1 if payment["paymentAmount"] < 0 else 0,
                        "currency_id": currency_id
                    })
                ],
    ...
    def mark_as_sync(self, customid):
        ...
        request_type = "patch"
        data = {"sync": True}
        response, content = self._embat_request(
